...
def collect_report_sections(model: Any, outputs: Any) -> List[ReportSection]:
    ...
    ai_notes: List[str] = []
    if getattr(outputs, "ai_insights", None) is not None:
        insight = outputs.ai_insights
        if getattr(insight, "ml_forecast", None) is not None:
            key_tables.append(ReportTable("AI Revenue Forecast", insight.ml_forecast))
        if getattr(insight, "generative_summary", None):
    ...
    sections.append(
        ReportSection(
            "Financial Position",
            [ReportTable("Statement of Financial Position", outputs.balance_sheet.rounded(0))],
        )
    )
    ...
    sections.append(
        ReportSection(
            "Cash Flow Statement",
            [ReportTable("Statement of Cash Flows", outputs.cash_flow.rounded(0))],
        )
    )

    sensitivity_tables: List[ReportTable] = []
    for name, table in getattr(outputs, "sensitivity_results", {}).items():
    ...
    for name, table in getattr(outputs, "scenario_results", {}).items():
        scenario_tables.append(ReportTable(f"Scenario: {name}", table))
    for key, result in getattr(outputs, "scenario_tool_results", {}).items():
...
def _build_json(sections: Sequence[ReportSection]) -> bytes:
    ...
    for section in sections:
        entry: MutableMapping[str, Any] = {"title": section.title, "tables": []}
        if section.notes:
            entry["notes"] = list(section.notes)
        for table in section.tables:
            entry["tables"].append(
                {"title": table.title, "rows": _table_to_rows(table.data), **({"note": table.note} if table.note else {})}
            )
...
# lower-level helpers for sheet/XML/csv etc. (unchanged from previous implementation)
def _collect_report_blocks(sections: Sequence[ReportSection], *, title: str | None = None) -> List[Tuple[str, str]]:
    blocks: List[Tuple[str, str]] = [("title", title or "Financial Model Report"), ("subtitle", f"Generated on {datetime.now(UTC).isoformat()} UTC")]
    for section in sections:
...
def _collect_sheet_entries(sections: Sequence[ReportSection]) -> List[dict[str, Any]]:
    ...
    if not entries:
        entries.append({"sheet_name": "Report", "rows": [{"Message": "No data available."}], "note": None})
    return entries
...
def _excel_sheet_xml(rows: Sequence[Sequence[str]]) -> str:
    ...
    for r_idx, row in enumerate(rows, start=1):
        ...
        for c_idx, value in enumerate(padded, start=1):
            ...
            parts.append(f'<c r="{column}{r_idx}" t="inlineStr"><is><t xml:space="preserve">{escaped}</t></is></c>')
        parts.append("</row>")
...
def _build_word_fallback(blocks: Sequence[Tuple[str, str]]) -> bytes:
    buffer = StringIO()
    for kind, text in blocks:
        prefix = {"title": "# ", "subtitle": "## ", "heading1": "### ", "heading2": "#### ", "note": "> "}.get(kind, "")
