...
class AppModuleTestCase(unittest.TestCase):
    def setUp(self):
        ...
        if "financial_models.pharma_app" in sys.modules:
            del sys.modules["financial_models.pharma_app"]

        importlib.invalidate_caches()
        self.stub = stub
        self.app = importlib.import_module("financial_models.pharma_app")
...
class RerunHelperTest(AppModuleTestCase):
    ...
    def test_projection_horizon_dropdown_updates_years(self):
        payload = json.loads(
            Path("src/financial_models/pharma/data/default_inputs.json").read_text(
                encoding="utf-8"
            )
        )
...
class SubscriptionCacheTest(AppModuleTestCase):
    def setUp(self):
        ...
        patcher = mock.patch(
            "financial_models.pharma_app._SUBSCRIPTION_GATEWAY", _StubGateway(self.store)
        )
        patcher.start()
...
class UploadLoaderTests(unittest.TestCase):
    def setUp(self):
        if "financial_models.pharma_app" in sys.modules:
            importlib.reload(sys.modules["financial_models.pharma_app"])
            self.app = sys.modules["financial_models.pharma_app"]
        else:
            self.app = importlib.import_module("financial_models.pharma_app")
